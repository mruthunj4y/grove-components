module GroveComponents.Utils.Renderer exposing (renderContents, renderNav, toVirtualDom)

import GroveComponents.Utils.GroveHtmlAttributes as GroveHtmlAttributes exposing (HrefLinkType(..), href)
import GroveComponents.Utils.Highlighter as Highlighter exposing (highlight)
import GroveComponents.Utils.Markdown.Table exposing (Table(..))
import GroveComponents.Utils.Markdown.TableParser as TableParser
...
renderLink destination maybeTitle contents =
    ...
    Html.a
        (titleAttrs
            ++ otherAttrs
            ++ GroveHtmlAttributes.href navigationType destination
        )
        contents
            ...
            let
                ( className, sanitized ) =
                    if String.endsWith "[Optional]" s then
                        ...
                        ( "", s )
            in
            p [ GroveHtmlAttributes.class className ] [ text sanitized ]
