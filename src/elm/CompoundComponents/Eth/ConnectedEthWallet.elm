port module GroveComponents.Eth.ConnectedEthWallet exposing
    ( ChooseWalletState(..)
    , ConnectionState(..)
    , InternalMsg(..)
    , LedgerAcccountData
...
import GroveComponents.Eth.Ethereum exposing (Account(..), CustomerAddress(..), getCustomerAddressString, shortenedAddressString)
import GroveComponents.Eth.Network exposing (Network, networkFromId, networkId)
...
import GroveComponents.Utils.NumberFormatter exposing (formatTokenBalanceWithSymbol)
...
inCopyBackArrow isGroveChain =
    if isGroveChain then
...
termsView userLanguage isGroveChain =
    if isGroveChain then
        text ""
...
connectItemView userLanguage isGroveChain providerType =
    let
        connectItemClass =
            if isGroveChain then
...
chooseWalletView userLanguage isGroveChain ({ chooseWalletState } as model) =
    let
        walletCopy =
            case chooseWalletState of
                    ...
                    let
                        { headerDescriptions, lineDivider } =
                            if isGroveChain then
                                ...
                                { headerDescriptions =
                                    ...
                                    , p [ class "center-text" ] [ text (Translations.to_start_using_grove userLanguage) ]
                                    ]
                        ...
                        ledgerItem =
                            if isGroveChain then
                                ...
                                [ connectItemView userLanguage isGroveChain Ledger ]
                                    ++ lineDivider
                        ...
                        coinbaseWalletItem =
                            if isGroveChain then
                                ...
                                [ div [ class "line" ] []
                                , connectItemView userLanguage isGroveChain WalletLink
                                ]
                    in
                    div [ class "connect-wallet-copy connect-wallet-copy--small-top" ]
                        ([ markSpan isGroveChain
                            ...
                            ++ [ div [ class "connect-choices" ]
                                    ([ connectItemView userLanguage isGroveChain Metamask ]
                                        ++ lineDivider
                                        ++ ledgerItem
                                        ++ [ connectItemView userLanguage isGroveChain WalletConnect
                                           ]
                                        ++ coinbaseWalletItem
                                        ++ lineDivider
                                        ++ [ connectItemView userLanguage isGroveChain Tally ]
                                    )
                               ]
                            ++ [ termsView userLanguage isGroveChain ]
                        )
                ...
                ChooseLedgerAccount ->
                    selectLedgerAddressModal userLanguage isGroveChain model

                LoadingLegerAccounts ->
                    connectingModal userLanguage (Just Ledger) model isGroveChain

                AttemptingConnectToWallet ->
                    connectingModal userLanguage model.selectedProvider model isGroveChain

                LedgerConnectionError ->
                    ledgerConnectionErrorModal userLanguage isGroveChain model
...
connectingModal userLanguage maybeSelectedProvider ({ chooseWalletState } as model) isGroveChain =
    let
        ...
        ( accountChosenClass, selectAddressString, selectButton ) =
            case model.chooseLedgerAcccountState.choosenLedgerAccount of
                ...
                _ ->
                    ...
                    , button [ class "button main", GroveComponents.Utils.Markup.disabled ]
                        [ text (Translations.select userLanguage) ]
                    )
    in
    div [ class ("connect-wallet-copy" ++ showBorderClass) ]
    ...
    let
        decoder =
            Json.Decode.map3 LedgerAcccountData
                ...
                (Json.Decode.field "account" (Json.Decode.maybe GroveComponents.Eth.Decoders.decodeCustomerAddress))
                (Json.Decode.field "ethBalance" (Json.Decode.maybe GroveComponents.Eth.Decoders.decimal))
    in
    giveLedgerAccount
